...
namespace LMS.Domain.Validation
{
    public class MemberValidator : AbstractValidator<MemberRegisterDTO>
    {
        public MemberValidator()
        {
            ...
            RuleFor(m => m.FirstName).Matches(@"^[a-zA-Z ]+$").WithMessage(SD.ValidationMessage.UserMessage.NameRegex);
            RuleFor(m => m.LastName).Matches(@"^[a-zA-Z\s]+$").WithMessage(SD.ValidationMessage.UserMessage.NameRegex);
            RuleFor(m => m.Email).EmailAddress().WithMessage(SD.ValidationMessage.UserMessage.EmailRegex);
            RuleFor(m => m.PhoneNumber).Matches(@"^\d{10}$").WithMessage(SD.ValidationMessage.UserMessage.PhoneNumberRegex);
